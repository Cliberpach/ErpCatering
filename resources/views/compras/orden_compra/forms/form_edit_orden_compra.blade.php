<form action="" id="formActualizarOrdenCompra" method="post">    
    <div class="row">
        @csrf      
            ...
            <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
                <div class="row">
                    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12 mb-3">
                        ...
                        <input required  value="{{$orden_compra->fecha_entrega}}"  type="date" name="fecha_entrega" id="fecha_entrega" class="form-control">
                        <span class="fecha_entrega_error msgError"  style="color:red;"></span>
                    </div>
                    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12 mb-3">
                        <label for="igv" style="font-weight: bold;">IGV</label>
                        <div class="form-check">
                            <input @if ($orden_compra->precios_igv == 1) checked @endif 
                            id="igv" name="igv" class="form-check-input"  type="checkbox" value="{{$igv}}" >
                    ...
                    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12 mb-3">
                        ...
                        <select required name="moneda" id="moneda" data-placeholder="Seleccionar" class="select2_form" onchange="changeMoneda(this.value)">
                            <option value="PEN" 
                                @if ($orden_compra->moneda === 'PEN') selected @endif
                            >SOLES</option>

                            <option value="USD"
                                @if ($orden_compra->moneda === 'USD') selected @endif
                            >DÓLARES</option>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-3">
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select required name="terminos_entrega" id="terminos_entrega" data-placeholder="Seleccionar" class="select2_form">
                                <option @if ($orden_compra->terminos_entrega === "PUESTO EN OBRA") selected @endif 
                                    value="PUESTO EN OBRA">PUESTO EN OBRA</option>
                            </select> 
                        </div>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-3">
                        ...
                        <select required name="proveedor" id="proveedor" data-placeholder="Seleccionar" class="select2_form">
                            @foreach ($proveedores as $proveedor)
                                <option value="{{$proveedor->id}}" @if ($orden_compra->proveedor_id == $proveedor->id) selected @endif>
                                    {{$proveedor->tipo_documento_descripcion.':'.$proveedor->nro_documento.'-'.$proveedor->nombre}}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                        ...
                        <select required name="modalidad_pago" id="modalidad_pago" data-placeholder="Seleccionar" class="select2_form">
                            @foreach ($modalidades_pago as $modalidad_pago)
                                @if ($modalidad_pago->tipo == "CREDITO")
                                    <option value="{{$modalidad_pago->id}}" @if($orden_compra->modalidad_pago_id === $modalidad_pago->id) selected @endif>
                    ...
                    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12 mb-3">
                        ...
                        <select required name="tipo_doc" id="tipo_doc" data-placeholder="Seleccionar" class="select2_form">
                            <option @if($orden_compra->documento === "FACTURA") selected @endif value="FACTURA">FACTURA</option>
                            <option @if($orden_compra->documento === "BOLETA") selected @endif value="BOLETA">BOLETA</option>
                        </select>
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select required name="persona_contacto" id="persona_contacto" data-placeholder="Seleccionar" class="select2_form">
                                ...
                                @if ($supervisor->supervisor_id)
                                    <option value="{{$supervisor->supervisor_id}}" @if($orden_compra->persona_contacto_id === $supervisor->supervisor_id) selected @endif>
                                        {{$supervisor->supervisor_contacto.' - '.'(S)'}}
                                    </option>   
                                @endif

                                @foreach ($proyecto_personal as $personal)
                                    <option value="{{$personal->colaborador_id}}" @if($orden_compra->persona_contacto_id === $supervisor->supervisor_id) selected @endif>
                                        {{$personal->persona_contacto}}
                                    </option>
                                @endforeach
                            </select>    
                        </div>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-3">
                        ...
                        <div class="form-floating">
                            <textarea name="observacion" id="observacion" class="form-control" placeholder="Leave a comment here" maxlength="200">{{$orden_compra->observacion}}</textarea>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="table-responsive">
                                    @include('compras.orden_compra.tables.table_orden_compra_detalle')
                                </div>
                            </div>
                        </div>
                        <div class="row justify-content-end">
