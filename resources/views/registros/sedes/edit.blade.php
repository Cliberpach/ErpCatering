...
<div class="card-style settings-card-1 mb-30">
    ...
    <div class="card-body">
        @include('registros.sedes.forms.form_edit_sedes')
    </div>
    <div class="card-footer d-flex justify-content-between align-items-center">
        ...
        <div style="display:flex;">
            ...
            <button class="btn btn-primary" type="submit" form="formActualizarSede">
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    const formActualizarSede = document.querySelector('#formActualizarSede');
                    const formData = new FormData(formActualizarSede);
                    const id = @json($sede->id);
                    let urlUpdateSede = `{{ route('registros.sedes.update', ['id' => ':id']) }}`;
                    urlUpdateSede = urlUpdateSede.replace(':id', id);

                    const response = await fetch(urlUpdateSede, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': token,
