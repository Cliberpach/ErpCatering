    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
            @include('reutilizables.modals.proveedores.forms.form_create_proveedor')
        </div>
...
<script>
    ...
    function pintarSelect2Proveedores(lstProveedores){
        ...
        lstProveedores.forEach(function(proveedor) {
            $('#proveedor').append(
                $('<option></option>').val(proveedor.id).text(`${proveedor.tipo_documento_descripcion}:${proveedor.nro_documento}-${proveedor.nombre}`)
            );
        });
    ...
    async function getProveedoresActualizados(){
        try {
            toastr.clear();
            const token                     =   document.querySelector('input[name="_token"]').value;
            const urlGetListProveedores     =   @json(route('compras.proveedor.getListProveedores'));
